...
from aether.dynamics import dcm_from_quaternion, quaternion_derivative
...
class FlightConfiguration:
    ...
    r"""Principal moments of inertia :math:`(I_{xx}, I_{yy}, I_{zz})`, kg m².

    ``None`` keeps the attitude **torque-free**: the quaternion integrates a
    constant angular rate and no aerodynamic moment acts. That was the only
    behaviour available, and it meant the vehicle had an attitude that never
    responded to anything -- no trim, no static margin, no pitch oscillation,
    and therefore none of the re-entry attitude dynamics the rest of this
    package computes conditions for.

    Supplying it, together with an :attr:`aero_table` that carries a pitching
    moment, turns on Euler's equations and the body-axis force. Both are
    required: a moment coefficient with no inertia has nothing to accelerate,
    and an inertia with no moment coefficient has nothing to be accelerated by.
    """

    reference_length: float | None = None
    """Length the pitching-moment coefficient is non-dimensionalised on, m.

    Defaults to :attr:`length` when a table is supplied without one. It must be
    the length the *table* used -- a moment coefficient carried on one length
    and applied on another is wrong by their ratio, silently.
    """

    pitch_damping: float = 0.0
    r""":math:`C_{m_q}`, per radian, **negative** for a damped vehicle.

    Zero leaves the pitch oscillation undamped, which is the honest default: a
    dynamic derivative cannot be had from the steady runs that build the rest of
    the table, and inventing one would put a number into the answer that no
    measurement supports. An undamped oscillation is visibly wrong; an invented
    damping is invisibly wrong.
    """
...
class FlightSimulator:
    ...
    def state_at(
        ...
        angular_rate: ArrayLike | None = None,
    ) -> _FloatArray:
        ...
        ``quaternion`` and ``angular_rate`` default to the identity attitude at
        rest. They are settable because the attitude now **responds** to the
        air: a vehicle that enters at incidence, or spinning, flies a different
        trajectory from one that enters aligned. Until there were aerodynamic
        moments there was no way for that to matter, and therefore no reason to
        be able to say it.
        """
        ...
        state = GlobalState(
            ...
            angular_rate=(
                np.zeros(3) if angular_rate is None else np.asarray(angular_rate, dtype=np.float64)
            ),
            mass=float(mass),
    ...
    def rhs(self, t: float, y: _FloatArray, burn: Burn | None = None) -> _FloatArray:
        ...
        #
        # Skipped entirely when the body-axis block below is active, because
        # that one produces the axial force itself. Running both would apply
        # the drag **twice** -- once resolved along the velocity and once along
        # the body axis -- and the result would look like a plausible
        # trajectory for a vehicle with half the ballistic coefficient.
        six_dof = cfg.aero_table is not None and cfg.inertia is not None
        accel = gravitational_acceleration(state.position, self._gravity)
        if speed > 0.0 and not six_dof:
        ...
        # --- body-axis aerodynamics, when there is a table and an inertia
        #
        # Drag along the relative velocity is a point mass with a spare
        # quaternion. A vehicle with a pitching-moment coefficient and an
        # inertia tensor has an attitude that *responds*: it trims, it
        # oscillates about trim, and its normal force turns the trajectory.
        # Both are required and neither is invented -- without a table there is
        # no C_m to apply, and without an inertia there is nothing for it to
        # accelerate, so the torque-free path below is entered unchanged.
        incidence = 0.0
        moment = np.zeros(3)
        if six_dof and speed > 0.0:
            assert cfg.aero_table is not None  # narrowed by six_dof
            dcm = dcm_from_quaternion(state.quaternion)
            body_velocity = dcm @ state.velocity
            axial_speed = float(body_velocity[0])
            # Total incidence about the body x axis, and the plane it acts in.
            lateral = float(np.hypot(body_velocity[1], body_velocity[2]))
            incidence = float(np.arctan2(lateral, axial_speed))
            coefficients = cfg.aero_table.at(mach, np.degrees(incidence))
            reference_length = float(cfg.reference_length or cfg.length)

            # Force: C_A along -x_body (aft is positive C_A, so it decelerates),
            # C_N in the plane of incidence, away from the oncoming flow.
            if lateral > 1.0e-12:
                normal_axis = np.array([0.0, body_velocity[1], body_velocity[2]])
                normal_axis = normal_axis / lateral
            else:
                normal_axis = np.zeros(3)
            force_body = (
                q_dyn
                * cfg.aero_table.reference_area
                * (
                    -coefficients.axial * np.array([1.0, 0.0, 0.0])
                    - coefficients.normal * normal_axis
                )
            )
            accel = accel + (dcm.T @ force_body) / max(float(state.mass), 1e-6)

            # Moment: C_m about the reference point, plus whatever pitch damping
            # was measured. The damping term carries the standard L/2V
            # non-dimensionalisation of the rate.
            pitch_axis = np.cross(np.array([1.0, 0.0, 0.0]), normal_axis)
            pitch_norm = float(np.linalg.norm(pitch_axis))
            if pitch_norm > 1.0e-12:
                pitch_axis = pitch_axis / pitch_norm
                rate = float(state.angular_rate @ pitch_axis)
                damping = cfg.pitch_damping * rate * reference_length / (2.0 * max(speed, 1.0e-6))
                moment = (
                    q_dyn
                    * cfg.aero_table.reference_area
                    * reference_length
                    * (coefficients.pitching_moment + damping)
                ) * pitch_axis
        ...
        if cfg.inertia is None or cfg.aero_table is None:
            out[layout.angular_rate] = 0.0  # torque-free: no moment to apply
        else:
            # Euler's equations in principal axes. The gyroscopic term is not
            # optional on a spinning re-entry vehicle: it is what couples roll
            # into pitch and yaw and what makes roll resonance a phenomenon
            # rather than an algebraic curiosity.
            inertia = np.asarray(cfg.inertia, dtype=np.float64)
            omega_body = np.asarray(state.angular_rate, dtype=np.float64)
            gyroscopic = np.cross(omega_body, inertia * omega_body)
            out[layout.angular_rate] = (moment - gyroscopic) / inertia
