...
def surface_of_revolution(
    station: Sequence[float] | _FloatArray,
    radius: Sequence[float] | _FloatArray,
    n_circ: int = 48,
    reference_fraction: float = 0.5,
) -> PanelModel:
    """Revolve a meridian ``(x, r)`` into a panelled surface.

    The general case that :func:`blunted_multiconic` is one parameterisation
    of. A body whose profile is not a cap plus frusta -- a sphere, an ogive, a
    tabulated contour -- has no way to be built through that function except by
    degenerating a segment, and a degenerate segment is a **sliver ring**: a
    hemisphere closed by a 0.1-degree cone carries one band of panels 78 um
    long against 2.5 mm around, which survives into the volume mesh as a ring
    of cells at a scaled Jacobian of 0.03 and accounts for essentially all of
    its poor cells.

    ``radius`` may start or end at zero -- a nose or tail point -- and the
    degenerate triangles there are dropped rather than kept at zero area.
    """
    station = np.asarray(station, dtype=np.float64)
    radius = np.asarray(radius, dtype=np.float64)
    if station.shape != radius.shape or station.ndim != 1:
        raise ValueError(
            f"station and radius must be matching 1-D arrays, got "
            f"{station.shape} and {radius.shape}"
        )
    if len(station) < 2:
        raise ValueError("a meridian needs at least two points")
    if np.any(radius < 0.0):
        raise ValueError("a meridian radius cannot be negative")
    if int(n_circ) < 3:
        raise ValueError(f"a revolved surface needs at least three stations, got {n_circ}")

    psi = np.linspace(0.0, 2.0 * np.pi, int(n_circ) + 1)
    vertices = np.empty((len(station), len(psi), 3), dtype=np.float64)
    vertices[:, :, 0] = station[:, None]
    vertices[:, :, 1] = radius[:, None] * np.cos(psi)[None, :]
    vertices[:, :, 2] = radius[:, None] * np.sin(psi)[None, :]

    ref_pt = np.array([reference_fraction * float(np.max(station)), 0.0, 0.0])
