...
#: Nodes each supported cell type carries, so the three places that walk an
#: element block agree on it by construction rather than by three copies.
_NODES_PER_CELL = {_TETRA: 4, _PRISM: 6, _HEX: 8}
_TRIANGLE = 5
_QUADRILATERAL = 9
...
class CaloricallyPerfect:
    ...
    several. :func:`~aether.cfd.config.regime_for` already switches the
...
def _hexahedra_to_tetrahedra(hexes: _IntArray) -> _IntArray:
    """Split each hexahedron into six tetrahedra.

    The six-tet decomposition rather than the five-tet one: five is possible but
    its diagonals do not agree between neighbouring cells, so a shared face gets
    split one way from one side and the other way from the other, and a mesh
    that was watertight as hexes stops being watertight as tetrahedra. Six is
    diagonal-consistent on the shared faces, which is what matters to everything
    downstream of this reader.
    """
    return np.concatenate(
        [
            hexes[:, [0, 1, 3, 4]],
...
def _read_elements(block: _IntArray, count: int) -> tuple[_IntArray, _IntArray, _IntArray]:
    """Split an element block into tetrahedra, prisms and hexahedra.
    ...
    """
    if block.size and int(block[0]) not in (_TETRA, _PRISM, _HEX):
        raise ValueError(
            f"element type {int(block[0])} is not a tetrahedron ({_TETRA}), a "
            f"prism ({_PRISM}) or a hexahedron ({_HEX}); no element type may be "
            "skipped, because a dropped element is a hole the picture renders "
            "as a void"
        )
    for nodes, kind in ((4, _TETRA), (6, _PRISM), (8, _HEX)):
        for stride in (nodes + 2, nodes + 1):
            if block.size == count * stride and np.all(block[::stride] == kind):
                ...
                empty6 = np.empty((0, 6), dtype=np.int64)
                empty8 = np.empty((0, 8), dtype=np.int64)
                if kind == _TETRA:
                    return cells, empty6, empty8
                if kind == _PRISM:
                    return empty4, cells, empty8
                return empty4, empty6, cells
    ...
    hexes: list[_IntArray] = []
    buckets = {_TETRA: tetrahedra, _PRISM: prisms, _HEX: hexes}
    cursor = 0
    for _ in range(count):
        kind = int(block[cursor])
        nodes = _NODES_PER_CELL.get(kind, -1)
        if nodes < 0:
            raise ValueError(
                f"element type {kind} is not a tetrahedron ({_TETRA}), a prism "
                f"({_PRISM}) or a hexahedron ({_HEX}); no element type may be "
                "skipped, because a dropped element is a hole the picture "
                "renders as a void"
            )
        buckets[kind].append(block[cursor + 1 : cursor + 1 + nodes])
    ...
    return (
        ...
        np.vstack(hexes) if hexes else np.empty((0, 8), dtype=np.int64),
    )
...
def read_su2_mesh(path: Path | str) -> Mesh:
    ...
    Handles the three volume element types these cases produce -- tetrahedra
    from the isotropic domains, prisms from the boundary-layer extrusion, and
    hexahedra from the afterbody block, whose annulus is swept from the
    forebody's own quadrilateral outflow faces so that the interface between
    the two blocks is one quad against one quad. Both triangular and
    quadrilateral boundary faces are read. Prisms and hexahedra are split into
    tetrahedra and quadrilaterals into triangles here, so callers see one
    element type regardless of how the mesh was built.
    ...
    tetrahedra, prisms, hexes = _read_elements(
    ...
    for _ in range(marker_count):
        ...
        # Triangles carry four tokens, quadrilaterals five, so the stride is
        # not fixed. A shock-fitted extrusion writes its downstream boundary as
        # quads because a prism's side face *is* one -- splitting it into
        # triangles for the solver would stop the boundary face matching the
        # volume face it belongs to. Splitting it *here* is fine and necessary,
        # because this mesh is for drawing and drawing wants triangles.
        # A marker may hold **both** kinds. The afterbody block gives the
        # farfield a triangular part, from the forebody's outer level, and a
        # quadrilateral part, from the wake's flared outer skin -- one boundary,
        # two element types, because the two blocks are built differently and
        # meet along it. Striding uniformly through that reads garbage, so this
        # walks the block when it is mixed and strides only when it is not.
        width_of = {_TRIANGLE: 4, _QUADRILATERAL: 5}
        first = int(tokens[cursor]) if faces else _TRIANGLE
        if first not in width_of:
            raise ValueError(
                f"{path}: marker {tag!r} holds element type {first}, which is "
                "neither a triangle nor a quadrilateral"
            )
        uniform = faces * width_of[first]
        table = None
        if cursor + uniform <= len(tokens):
            candidate = (
                np.array(tokens[cursor : cursor + uniform], dtype=np.int64).reshape(
                    faces, width_of[first]
                )
                if faces
                else None
            )
            if candidate is None or np.all(candidate[:, 0] == first):
                table = candidate
        if table is not None:
            nodes = table[:, 1:] if faces else np.empty((0, 3), dtype=np.int64)
            if first == _QUADRILATERAL and faces:
                nodes = np.concatenate([nodes[:, [0, 1, 2]], nodes[:, [0, 2, 3]]])
            markers.setdefault(tag, []).append(nodes)
            cursor += uniform
        else:
            triangles: list[_IntArray] = []
            for _ in range(faces):
                kind = int(tokens[cursor])
                width = width_of.get(kind)
                if width is None:
                    raise ValueError(
                        f"{path}: marker {tag!r} holds element type {kind}, "
                        "which is neither a triangle nor a quadrilateral"
                    )
                face = np.array(tokens[cursor + 1 : cursor + width], dtype=np.int64)
                if kind == _QUADRILATERAL:
                    triangles.append(face[[0, 1, 2]])
                    triangles.append(face[[0, 2, 3]])
                else:
                    triangles.append(face)
                cursor += width
            markers.setdefault(tag, []).append(
                np.vstack(triangles) if triangles else np.empty((0, 3), dtype=np.int64)
            )
    ...
    if hexes.size:
        volume.append(_hexahedra_to_tetrahedra(hexes))
    if not volume:
