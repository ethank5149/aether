...
def freestream_direction(alpha: float = 0.0, beta: float = 0.0) -> _FloatArray:
    r"""Unit freestream vector in body axes, from incidence and sideslip (rad).

    SU2's own convention, because the mesh has to agree with the solve:

    .. math::

        \hat v = (\cos\alpha\cos\beta,\ \sin\beta,\ \sin\alpha\cos\beta)

    At zero incidence and sideslip this is :math:`(1, 0, 0)` exactly, so
    anything sized against it keeps the axial behaviour unchanged.
    """
    ca, sa = np.cos(float(alpha)), np.sin(float(alpha))
    cb, sb = np.cos(float(beta)), np.sin(float(beta))
    return np.asarray([ca * cb, sb, sa * cb], dtype=np.float64)
...
def stagnation_refinement(
    ...
    *,
    ...
    r"""A :class:`RefinementBall` resolving a blunt nose's captured bow shock.

    Places the region at the body's most upstream point **with respect to the
    oncoming flow** and sizes it from :func:`shock_layer_cell_size`, extending
    ``reach`` standoffs back so the shock is resolved where it stands as well
    as at the wall.
    ...
    ``direction``
        Freestream unit vector in body axes, from :func:`freestream_direction`.
        Defaults to :math:`(1,0,0)`.

        **This is not cosmetic.** The stagnation point is where the flow first
        meets the body, which is the axial apex *only at zero incidence*. At
        angle of attack it migrates windward, and a region centred on the axial
        apex leaves the real stagnation region — where the gradients are
        steepest — at the background cell size. Measured consequence: Mach 8
        sphere-cone and biconic cases at :math:`\alpha = +2^\circ` diverged
        during the **first-order** startup, at iteration 74 and 93, on meshes
        whose :math:`\alpha = 0` counterparts converged without trouble.
    """
    ...
    if not np.isfinite(norm) or norm <= 0.0:
        raise ValueError(f"direction must be a non-zero vector, got {direction!r}")
    flow = flow / norm
    # The first point the flow reaches is the one least far along its
    # direction. At (1,0,0) this is argmin(x) -- the previous behaviour,
    # exactly.
    apex = vertices[int(np.argmin(vertices @ flow))]
