...
__all__ = ["cfd_results", "reference_root", "results_root"]
...
#: Names the reference share: transcribed figure data, published tables, and
#: the third-party papers behind them. Bind-mounted in the devcontainer, and
#: elsewhere wherever the reader keeps it.
REFERENCE_ENV = "AETHER_REFERENCE"

#: Used when :data:`REFERENCE_ENV` is unset, for the same reason as above.
DEFAULT_REFERENCE = Path("reference")
...
def reference_root() -> Path:
    """The root of the reference share."""
    named = os.environ.get(REFERENCE_ENV)
    return Path(named) if named else DEFAULT_REFERENCE
