...
def great_circle_range(origin: GeodeticPosition, destination: GeodeticPosition) -> float:
    ...
    A two-body Keplerian model on a sphere of this radius -- true anomaly,
    Kepler's equation, ``body_radius = WGS84_MEAN_RADIUS`` -- closes a range
    loop by converting a range to a central angle. Feeding it an ellipsoidal
    geodesic instead mixes two models, and a trajectory that had reached its
    target point to under a metre then missed by **15.6 km**. The ellipsoidal
    number is the more accurate description of the ground; it is the less
    accurate input to a spherical propagator, and both statements are true at
    once.
    """
